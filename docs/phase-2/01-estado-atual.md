...
SPB-265 (estado persistente descartavel + telemetria do Collector) **CONCLUIDO — MERGEADO e VALIDADO pelo Scheduled Task real em 21/08/2026**, PR #11, Issue #10, merge commit `f2b026bc16946bab3c424761e6b1b65917e5d616`: estado local em `data/collector/collector_state.json`, ignorado pelo Git, com `schema_version=1`, `last_exit_code=0`, `last_success_at=2026-08-22T00:56:40Z`, `recent_ids_count=100`; telemetria `COLLECTOR_TELEMETRY` observada em ciclos reais com `exit_code=0`, `pages_attempted=10`, `pages_ok=10`, `parser_ok=10`, `projects_unique=100`, ingest `received=100`, estado `loaded`/`written`. Scheduled Task `SmartPayBot Collector`: `LastTaskResult=0`, `NumberOfMissedRuns=0`, execucao sem intervencao manual. Nao houve deploy VPS, alteracao de Scheduled Task, cadencia, `--pages 10`, early-stop, shadow mode, matcher/notifier, Telegram ou ingest server-side.

SPB-266 (shadow mode de coleta incremental) **MERGEADO e com primeiro ciclo operacional real PASS em 24/08/2026**, PR #14, Issue #13 ainda aberta, merge commit `523920f7e92e5e2c8f4911e4fa232118bca94391`: Scheduled Task `SmartPayBot Collector` executou sem intervencao manual (`LastTaskResult=0`, `NumberOfMissedRuns=0`), preservando coleta real de 10 paginas (`pages_attempted=10`, `pages_ok=10`, `parser_ok=10`, `projects_unique=100`, ingest `received=100`) e estado local `loaded`/`written`. A telemetria `COLLECTOR_TELEMETRY` passou a registrar campos `shadow_*`; primeiro ciclo observado: `shadow_enabled=true`, `shadow_state_status=loaded`, `shadow_hypothetical_stop_page=2`, `shadow_pages_saved_hypothetical=8`, `shadow_missed_new_if_active=0`, `shadow_known_projects=99`, `shadow_unknown_projects=1`, `shadow_known_ratio=0.99`, `shadow_cycle_usable=true`, `shadow_reason=stop_found`. Janela de observacao iniciada; nao houve deploy VPS, alteracao de Scheduled Task, cadencia, `--pages 10` ou early-stop real.

SPB-268 (Collector Fast/Deep) **CONCLUIDO — MERGEADO e HOMOLOGADO operacionalmente**: PR #42 introduziu `--mode auto|fast|deep`, `--fast-pages` e `--deep-pages`; PR #43 ativou o runner versionado em `--mode auto --fast-pages 2 --deep-pages 10`; o cutover da Scheduled Task foi validado em PT5M com `Duration=P3650D`, `MultipleInstances=IgnoreNew`, `ExecutionTimeLimit=PT15M` e `WorkingDirectory=D:\Projetos\SmartPayBot`. Homologacao C3 em AUTO+PT10M comprovou dois ciclos DEEP completos; homologacao C4 em AUTO+PT5M comprovou a sequencia `DEEP -> FAST -> DEEP`. FAST coletou 2 paginas com `pages_ok=2`, `parser_ok=2`, `exit_code=0`, ingest coerente, `collector_deep_complete=false`, shadow inutilizavel (`shadow_reason=fast_cycle_not_full_scan`, `shadow_missed_new_if_active=null`), `last_deep_started_at` sem avancar e snapshot/watermark DEEP preservados. DEEP coletou 10 paginas com parser 10/10, ingest coerente, `collector_deep_complete=true` e `last_deep_started_at` avancando somente apos DEEP completo. Baseline operacional atual: paginas 1-2 aproximadamente a cada 5 min, paginas 1-10 aproximadamente a cada 10 min, ~12 GETs de listagem/10 min (+20% sobre o baseline anterior). Early-stop nao foi ativado; SPB-267 permanece bloqueado; PT2M nao esta autorizado; SPB-269 segue necessario antes de qualquer cadencia mais agressiva.
